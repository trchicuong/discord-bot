...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (metadataThread && !metadataThread.archived) return inter.editReply({ content: await Translate(`Lyrics thread already created <${inter.member}> !`) }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
        ...
        return inter.editReply({ content: await Translate(`Successfully syncronized lyrics in <${thread.name}> ! <${inter.member}> <✅>`) });
    }
}
