...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (channel.type !== ChannelType.GuildText) return inter.editReply({ content: await Translate(`You need to send it to a text channel.. <❌>`) }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
        ...
        inter.editReply({ content: await Translate(`Sending controller to <${channel.url}>... <✅>`) });
