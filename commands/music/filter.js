...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (!filter) return inter.editReply({ content: msg }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
        ...
        return inter.editReply({ embeds: [filterEmbed] }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
    }
}