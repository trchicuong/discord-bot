...
module.exports = async ({ inter, queue }) => {
    ...
    inter.member.send({ embeds: [embed] })
        .then(async () => {
            return inter.editReply({ content: await Translate(`I have sent you the title of the music by private messages <✅>`) }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
        }).catch(async (error) => {
            console.error(error);
            return inter.editReply({ content: await Translate(`Unable to send you a private message... try again ? <❌>`) }).then(() => setTimeout(() => inter.deleteReply() ,3000 ));
        });
}
